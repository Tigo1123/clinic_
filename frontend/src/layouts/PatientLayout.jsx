...
export default function PatientLayout() {
  ...
  return <div className="patient-app"><header className="patient-header"><NavLink to="/patient" className="patient-brand">{t('brandName')}</NavLink><div><button className="patient-button secondary" onClick={toggleLanguage}>{i18n.language === 'ar' ? 'English' : 'العربية'}</button><button className="patient-button secondary" onClick={logout}>{t('logout')}</button></div></header><nav className="patient-nav" aria-label={t('patientNavigation')}>{links.map(([to,label]) => <NavLink key={to} to={to} end={to === '/patient'}>{label}</NavLink>)}</nav><main className="patient-main"><Outlet /></main></div>;
}
