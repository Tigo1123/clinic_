...
export default function PatientLayout() {
  ...
  return <div className="patient-app"><header className="patient-header"><NavLink to="/patient" className="patient-brand"><span><HeartPulse/></span>{t('brandName')}</NavLink><div><button className="patient-icon-button" aria-label={t('notifications')}><Bell/></button><button className="patient-icon-button" onClick={toggleLanguage} aria-label="Change language"><Languages/></button><button className="patient-icon-button" onClick={async ()=>{await logout();navigate('/patient-login')}} aria-label={t('logout')}><LogOut/></button></div></header><nav className="patient-nav" aria-label={t('patientNavigation')}>{links.map(([to,label,Icon],index)=><NavLink key={to} to={to} end={to==='/patient'} className={index===2?'patient-nav__primary':''}><Icon/><span>{label}</span></NavLink>)}</nav><main className="patient-main"><Outlet/></main></div>;
}
