...
import { CalendarDays, Check, Eye, EyeOff, FileText, HeartPulse, LockKeyhole, Pill } from 'lucide-react';
...
function AuthShell({title,children,onboarding=false}){const{t,i18n}=useTranslation();const isOnboarding=onboarding||title===t('createPatientAccount');const languageControl=<div className={isOnboarding?'onboarding-language':'patient-auth-language'} role="group" aria-label="Language"><button type="button" className={i18n.language==='ar'?'active':''} onClick={()=>i18n.changeLanguage('ar')} lang="ar">العربية</button><button type="button" className={i18n.language!=='ar'?'active':''} onClick={()=>i18n.changeLanguage('en')} lang="en">English</button></div>;return <main className={`patient-auth-shell${isOnboarding?' patient-auth-shell--onboarding':''}`}><aside className="patient-auth-aside"><Link className="patient-auth-brand" to="/"><span><HeartPulse size={21}/></span>{t('brandName')}</Link><div className="patient-auth-hero-copy"><span className="patient-auth-kicker"><HeartPulse size={15}/>{t('patientPortal')}</span><h2>{title}</h2><p>{t('secureAccessDescription')}</p><ul className="patient-auth-trust" aria-label={t('patientPortal')}><li><CalendarDays size={17}/>{t('myAppointments')}</li><li><Pill size={17}/>{t('prescriptions')}</li><li><FileText size={17}/>{t('labResults')}</li></ul></div><div className="patient-auth-doctor" aria-hidden="true"><img src={patientAuthDoctor} alt=""/><span className="patient-auth-doctor__badge"><LockKeyhole size={16}/>{t('secureAccessDescription')}</span></div></aside><div className="patient-auth-content"><section className="patient-card patient-auth"><header className="patient-auth-card-header">{languageControl}<span className="patient-auth-card-kicker"><LockKeyhole size={14}/>{t('secureAccessDescription')}</span><h1>{title}</h1></header>{children}</section></div></main>}
