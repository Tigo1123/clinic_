...
import { INITIAL_ONBOARDING_FORM, NAME_FIELDS, ONBOARDING_STEPS, onboardingErrorMessage, passwordChecks, registrationPayload, validateOnboardingStep } from './onboarding';
...
export function PatientRegister() {
  ...
  if (verified) return <AuthShell title={t('onboardingSuccessTitle')}><section className="onboarding-success" aria-live="polite"><Check size={44}/><h2>{t('onboardingSuccessTitle')}</h2><p>{t('onboardingSuccessText')}</p><strong>{identity?.fullNameAr || identity?.fullNameEn}</strong>{identity?.fileNumber && <p className="onboarding-mrn">{t('onboardingMrn')}: {identity.fileNumber}</p>}{error&&<Alert>{error}</Alert>}<button className="patient-button" disabled={loading} onClick={continueToDashboard}>{loading?t('loading'):t('onboardingContinueDashboard')}</button></section></AuthShell>;
  if (challenge) return <AuthShell title={t('verificationCode')}><form onSubmit={verify} aria-live="polite"><p>{t('verificationCodePrompt')}</p>{challenge.developmentCode && <div className="patient-alert success">{t('developmentCode')}: <strong>{challenge.developmentCode}</strong></div>}<Field label={t('verificationCode')} value={code} onChange={setCode} autoComplete="one-time-code"/>{error&&<Alert>{error}</Alert>}<button className="patient-button" style={{width:'100%'}} disabled={loading}>{loading?t('loading'):t('verify')}</button></form></AuthShell>;
  const nameFields = step === 0 ? NAME_FIELDS.slice(0, 4) : NAME_FIELDS.slice(4);
  return <AuthShell title={t('createPatientAccount')}><form className="onboarding-form" onSubmit={submit}><ol className="onboarding-progress" aria-label={t('onboardingProgress')}>{ONBOARDING_STEPS.map((key, index) => <li key={key} className={index === step ? 'active' : index < step ? 'complete' : ''}><button type="button" disabled={index > step} onClick={() => setStep(index)} aria-current={index === step ? 'step' : undefined}>{index + 1}<span>{title(key)}</span></button></li>)}</ol><h2>{title(ONBOARDING_STEPS[step])}</h2><p className="onboarding-caption">{t(`onboarding${ONBOARDING_STEPS[step][0].toUpperCase()}${ONBOARDING_STEPS[step].slice(1)}Help`)}</p>
  ...
  {step === 3 && <div className="onboarding-grid"><label className="patient-field">{t('phone')}<div className="patient-phone-control"><select value={form.phoneCountry} onChange={event => update('phoneCountry', event.target.value)} aria-label={t('selectCountry')} dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>{PATIENT_PHONE_COUNTRIES.map(country => <option key={country} value={country}>{countryFlag(country)} {countryName(country, i18n.language)} ({callingCode(country)})</option>)}</select><input type="tel" inputMode="tel" dir="ltr" value={form.phone} onChange={event => updatePhone(event.target.value)} placeholder={t('phoneExample')} autoComplete="tel-national" required aria-invalid={Boolean(fieldErrors.phone)}/></div></label>{fieldErrors.phone&&<span className="field-error">{fieldErrors.phone}</span>}<Field label={t('email')} type="email" value={form.email} onChange={value => update('email', value)} error={fieldErrors.email} autoComplete="email"/></div>}
  ...
  {error&&<Alert>{error}</Alert>}<div className="onboarding-actions">{step > 0 && <button type="button" className="patient-button secondary" onClick={() => setStep(current => current - 1)} disabled={loading}>{t('onboardingBack')}</button>}{step < 6 ? <button type="button" className="patient-button" onClick={next}>{t('onboardingNext')}</button> : <button className="patient-button" disabled={loading}>{loading?t('loading'):t('createAccount')}</button>}</div></form></AuthShell>;
}
...
function AuthShell({title,children,onboarding=false}){const{t,i18n}=useTranslation();const isOnboarding=onboarding||title===t('createPatientAccount');return <main className={`patient-auth-shell${isOnboarding?' patient-auth-shell--onboarding':''}`}><aside className="patient-auth-aside"><Link to="/"><HeartPulse size={24}/>{t('brandName')}</Link><div><h2>{title}</h2><p>{t('secureAccessDescription')}</p></div>{isOnboarding&&<HealthcareIllustration variant="doctor" className="onboarding-illustration"/>}</aside><div className="patient-auth-content"><section className="patient-card patient-auth">{isOnboarding&&<div className="onboarding-language" role="group" aria-label="Language"><button type="button" className={i18n.language==='ar'?'active':''} onClick={()=>i18n.changeLanguage('ar')} lang="ar">العربية</button><button type="button" className={i18n.language!=='ar'?'active':''} onClick={()=>i18n.changeLanguage('en')} lang="en">English</button></div>}<h1>{title}</h1>{children}</section></div></main>}
